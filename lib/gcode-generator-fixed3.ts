...
export function generateGCode(params: MachiningParams): string[] {
  ...
  // Calcular o recuo como 1mm menor que o raio inicial
  const recuoY = raioInicial - 1
  ...
  for (let ii = 0; ii < params.numEntalhes; ii++) {
    ...
    while (true) {
      ...
      if (deslocamentoX > 0) {
        ...
        // First retract to Y before raising Z - sempre volta para o mesmo ponto de recuo
        ...
        // Retract to Y before raising Z - sempre volta para o mesmo ponto de recuo
        codigoG.push(`G0 Y${recuoY.toFixed(2)}`)
        codigoG.push(`G0 Z${params.pontoInicioZ}`)
        codigoG.push("G0 X0")
      } else {
