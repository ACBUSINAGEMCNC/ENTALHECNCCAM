...
export function generateGCode(params: MachiningParams): string[] {
  ...
  if (params.chavetaConica) {
    ...
    // Determinar qual é o menor valor Y (inicial ou final)
    const menorY = Math.min(raioInicial, yFinalPrimeiroCorte);
    
    // Determinar o recuo com base no lado de corte
    if (params.ladoCorte === "positivo") {
      // Para lado positivo, recua 1mm menor que o menor valor Y
      pontoRecuoY = menorY - 1;
    } else {
      // Para lado negativo, recua 1mm maior que o maior valor Y
      const maiorY = Math.max(raioInicial, yFinalPrimeiroCorte);
      pontoRecuoY = maiorY + 1;
    }
  } else {
    // Para cortes não cônicos, usar a lógica padrão baseada no lado de corte
    if (params.ladoCorte === "positivo") {
      pontoRecuoY = raioInicial - 1;
    } else {
      pontoRecuoY = raioInicial + 1;
    }
  }

  // Verificar se estamos cortando de diâmetro maior para menor
  const cortandoAoContrario = params.diametroInicial > params.diametroFinal;
  
  // Se estamos cortando ao contrário, inverter a lógica do recuo
  if (cortandoAoContrario) {
    if (params.ladoCorte === "positivo") {
      // Se o lado de corte é positivo mas estamos cortando ao contrário,
      // o recuo deve ser como se fosse negativo
      pontoRecuoY = Math.max(raioInicial, raioFinal) + 1;
    } else {
      // Se o lado de corte é negativo mas estamos cortando ao contrário,
      // o recuo deve ser como se fosse positivo
      pontoRecuoY = Math.min(raioInicial, raioFinal) - 1;
    }
    
    // Se também temos chaveta cônica, precisamos considerar o Y final do primeiro corte
    if (params.chavetaConica) {
      const profundidadeZ = Math.abs(params.profundidadeFinal);
      const deslocamentoY = profundidadeZ * Math.tan(anguloRadianos);
      const yFinalPrimeiroCorte = raioInicial + deslocamentoY;
      ...
      if (params.ladoCorte === "positivo") {
        // Para lado positivo mas cortando ao contrário, recua 1mm maior que o maior valor Y
        pontoRecuoY = Math.max(raioInicial, yFinalPrimeiroCorte) + 1;
      } else {
        // Para lado negativo mas cortando ao contrário, recua 1mm menor que o menor valor Y
