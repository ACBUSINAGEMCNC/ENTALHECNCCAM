    ...
    while (true) {
      ...
      if (deslocamentoX > 0) {
        ...
        if (params.chavetaConica) {
          ...
          // Formato: G1 Y<valor_y> Z<valor_z> F<avanço> (Calculando para <ângulo>° graus)
          codigoG.push(`G1 Y${yAtual.toFixed(3)} Z${params.profundidadeFinal} F${params.avanco} (Calculando para ${params.anguloConico}° graus)`)
        } else {
      ...
      if (nextY < raioFinal) {
        ...
        if (deslocamentoX > 0) {
          ...
          if (params.chavetaConica) {
            // Movimento combinado Y e Z com ângulo cônico
            codigoG.push(`G1 Y${raioFinal.toFixed(3)} Z${params.profundidadeFinal} F${params.avanco} (Calculando para ${params.anguloConico}° graus)`)
          } else {
            codigoG.push(`G1 Z${params.profundidadeFinal} F${params.avanco}`)
          }
